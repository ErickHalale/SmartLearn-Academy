...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_fact'])) {
        ...
        if (!empty($fact_text)) {
            $stmt = $pdo->prepare("
                INSERT INTO secondary_did_you_know
    ...
    elseif (isset($_POST['delete_fact'])) {
        ...
        $stmt = $pdo->prepare("DELETE FROM secondary_did_you_know WHERE id = ?");
...
// Fetch all facts
$facts = $pdo->query("
    SELECT dyk.*, f.form_name, s.subject_name, t.topic_name 
    FROM secondary_did_you_know dyk 
    LEFT JOIN secondary_forms f ON dyk.form_id = f.id 
    LEFT JOIN secondary_subjects s ON dyk.subject_id = s.id 
    LEFT JOIN secondary_topics t ON dyk.topic_id = t.id 
    ORDER BY dyk.display_order, dyk.created_at DESC
")->fetchAll(PDO::FETCH_ASSOC);

// Fetch facts for dropdown
$facts = $pdo->query("SELECT id, fact_text FROM secondary_did_you_know ORDER BY display_order, created_at DESC")->fetchAll(PDO::FETCH_ASSOC);

// Fetch specific fact for editing
$edit_fact = null;
if (isset($_GET['edit'])) {
    $stmt = $pdo->prepare("
        SELECT * FROM secondary_did_you_know 
        WHERE id = ?
    ");
...
<head>
    ...
    <title>Manage Secondary Facts - SmartLearn</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1><i class="fas fa-lightbulb"></i> Manage Secondary Facts</h1>
            <p>Add, edit, and delete educational facts for secondary level students</p>
        </div>

        <div class="nav-links">
            <a href="admin.php"><i class="fas fa-tachometer-alt"></i> Dashboard</a>
            <a href="manage_facts_primary.php"><i class="fas fa-school"></i> Primary Facts</a>
                    ...
                    <form method="POST" id="factForm">
                        ...
                        <div class="form-group">
                            <?php if ($edit_fact): ?>
                                ...
                                <a href="manage_facts_secondary.php" class="btn btn-secondary">
    ...
    <script>
        function editFact() {
            ...
            if (factId) {
                window.location.href = 'manage_facts_secondary.php?edit=' + factId;
            } else {
