...
// Add status column to secondary_users table if it doesn't exist
try {
    $pdo->exec("ALTER TABLE secondary_users ADD COLUMN status ENUM('active', 'inactive') DEFAULT 'active'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_status'])) {
        ...
        if (in_array($new_status, ['active', 'inactive'])) {
            try {
                $stmt = $pdo->prepare("UPDATE users SET status = ? WHERE id = ? AND education_level = 'secondary'");
        ...
        if (!empty($selected_pupils) && in_array($action, ['activate', 'deactivate'])) {
            try {
                ...
                $stmt = $pdo->prepare("UPDATE users SET status = ? WHERE id IN ($placeholders) AND education_level = 'secondary'");
...
$where_conditions = ["education_level = 'secondary'"];
$params = [];
...
try {
    ...
    $pupils_stmt = $pdo->prepare("
        SELECT id, first_name, last_name, username, school_name, form, status, created_at
...
<head>
    ...
    <title>View Secondary Pupils - SmartLearn Admin</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="header">
        <div class="logo-section">
            <h1>View Secondary Pupils (<?php echo $total_pupils; ?>)</h1>
        </div>
        <div class="nav-links">
    ...
    <div class="container">
        ...
        <div class="card">
            <h2>Secondary School Pupils</h2>

            <!-- Filter Tabs -->
            <div class="filter-tabs">
                <a href="?filter=all&search=<?php echo urlencode($search); ?>&form=<?php echo $form_filter; ?>" 
                   class="filter-tab <?php echo $filter === 'all' ? 'active' : ''; ?>">
                    All Pupils (<?php echo $total_pupils; ?>)
                </a>
                <a href="?filter=active&search=<?php echo urlencode($search); ?>&form=<?php echo $form_filter; ?>" 
                   class="filter-tab <?php echo $filter === 'active' ? 'active' : ''; ?>">
                    Active (<?php echo $active_pupils; ?>)
                </a>
                <a href="?filter=inactive&search=<?php echo urlencode($search); ?>&form=<?php echo $form_filter; ?>" 
            ...
            <div class="filters-section">
                <form method="GET" style="display: flex; flex-wrap: wrap; gap: 15px; align-items: center; width: 100%;">
                    ...
                    <div class="filter-group">
                        ...
                        <select name="form" onchange="this.form.submit()">
                            ...
                            <?php foreach ($forms as $form): ?>
                                <option value="<?php echo $form['form_number']; ?>" 
                                    <?php echo $form_filter == $form['form_number'] ? 'selected' : ''; ?>>
                    ...
                    <a href="view_pupils_secondary.php" class="btn btn-small btn-secondary">Clear Filters</a>
                </form>
            </div>
                        ...
                        <tbody>
                            <?php foreach ($pupils as $pupil): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="form-badge">Form <?php echo $pupil['form']; ?></span>
                                    </td>
                                    <td>
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>
                        <?php if (!empty($search) || $form_filter > 0 || $filter !== 'all'): ?>
                            No pupils match your current filters. Try adjusting your search criteria.
                        <?php else: ?>
                            No secondary school pupils have registered yet.
                        <?php endif; ?>
                    </p>
                    <?php if (!empty($search) || $form_filter > 0 || $filter !== 'all'): ?>
                        <a href="view_pupils_secondary.php" class="btn" style="margin-top: 15px;">Clear Filters</a>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
